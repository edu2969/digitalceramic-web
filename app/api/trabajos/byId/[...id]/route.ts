...
export async function GET(
  ...
  try {
    ...
    const { data: trabajoData, error: trabajoError } = await supabase
      .from("trabajos")
      .select(
        "id, fecha_envio, fecha_entrega, monto, notas, nombre_clinica, direccion_despacho, " + 
        "estado, enviado_por, url_superior, url_inferior, url_mordida, url_gingival, archivo_superior, " + 
        "archivo_inferior, archivo_mordida, archivo_gingival, " + 
        "pacientes (nombre, apellido, fecha_nacimiento), " + 
        "profiles (user_id, nombre, apellido, numero_registro, rut), " + 
        "piezas (numero, paleta, colores, tipo, tibase_cementado, tibase_plataforma, tibase_gingival, conexion)"
      )
      .eq("id", trabajoId)
      .maybeSingle()

    if (trabajoError || !trabajoData) {
      console.log("Error - ", trabajoError)
      return NextResponse.json(
    ...
    const { data: perfilOdontologo, error: errorPerfilOdontologo } = await supabase
    .from("profiles")
    .select("id, user_id")
    .eq("user_id", user.id)
    .single();

    if(!perfilOdontologo || errorPerfilOdontologo) {
      console.log("[GET] trabajos/byId", id, perfilOdontologo, errorPerfilOdontologo);
      return NextResponse.json({ ok: false, error: "No se obtuvo el perfil del odontologo"}, { status: 400 })
    }

    const { data: todosLosTrabajos, error: errorTodosLosTrabajos } = await supabase
    .from("trabajos")
    .select("id")
    .eq("profile_id", perfilOdontologo.id)
    .neq("id", trabajo.id);

let cantidadPiezasHechas = 0;

if (todosLosTrabajos && todosLosTrabajos.length > 0) {
    const { count, error: errorPiezasHechas } = await supabase
        .from("piezas")
        .select("*", { count: 'exact', head: true })
        .in("trabajo_id", todosLosTrabajos.map((t) => t.id));
    
    if (!errorPiezasHechas) {
        cantidadPiezasHechas = count || 0;
    }
}
    ...
    return NextResponse.json({
      ...
      direccion: trabajo.direccion_despacho,
